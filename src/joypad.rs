...
impl TrapHandler for Joypad {
    fn read(&self, addr: u16) -> Result<u8, String> {
        ...
        if (self.select_buttons && self.state.button_b)
            || (self.select_directions && self.state.left)
        {
            val &= !(1 << 1);
        }
        if (self.select_buttons && self.state.button_a)
            || (self.select_directions && self.state.right)
        {
            val &= !(1 << 0);
        }
        Ok(val)
